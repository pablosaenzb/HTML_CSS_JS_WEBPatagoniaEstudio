...
package com.emerald.phlex.services {
	...
	public class XMLParser {
		...
		private var __root:*;
		private var servicesArray:Array;
	...
	//	Function. Parses the attributes of the "settings" node.
		...
		public function settingsNodeParser(node:XMLList):void {
			...
			for each (var attr:* in node.@*) {
				...
				switch (attr_name) {
					case "servicesType":
						if (attr == "1" || attr == "2") __root.servicesType = attr;
					break;
					...
					break;
					case "itemVSpacing":
						if (Number(attr) >= 0 && attr != "") __root.itemVSpacing = attr;
					break;
					...
					break;
					case "imageTopMargin":
						if (Number(attr) >= 0 && attr != "") __root.imageTopMargin = attr;
					break;
					case "imageBottomMargin":
						if (Number(attr) >= 0 && attr != "") __root.imageBottomMargin = attr;
					break;
					case "imageRightMargin":
						if (Number(attr) >= 0 && attr != "") __root.imageRightMargin = attr;
					break;
	...
	/****************************************************************************************************/
	//	Function. Parses the elements of the "services" node.
	
		public function servicesNodeParser(node:XML):Array {
			servicesArray = new Array();
			for each (var aChild:XML in node.*) {
				if (aChild.name() == "service" && aChild.hasComplexContent()) {
					var serviceObject:Object = new Object();
					if (aChild.title.text() != "" && aChild.title.text() != undefined) serviceObject.title = aChild.title.text();
					if (aChild.image.@src != "" && aChild.image.@src != undefined) serviceObject.imgSrc = aChild.image.@src;
					if (aChild.image.@clickLink != "" && aChild.image.@clickLink != undefined) serviceObject.clickLink = aChild.image.@clickLink;
					if (aChild.image.@clickTarget != "" && aChild.image.@clickTarget != undefined) serviceObject.clickTarget = aChild.image.@clickTarget;
					if (aChild.content.hasComplexContent()) serviceObject.blocks = contentNodeParser(aChild.content);
	...
	/****************************************************************************************************/
	//	Function. Parses the elements of a "service" -> "content" node.
