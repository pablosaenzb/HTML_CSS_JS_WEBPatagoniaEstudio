...
class ThemifyBuilder_Data_Manager {
	...
	private $regex_static_content = '/(?:<p>)?(?:<|&lt;)!--themify_builder_static--(?:>|&gt;)(?:.*?)?(?:<|&lt;)!--\/themify_builder_static--(?:>|&gt;)(?:<\/p>)?/s';
	...
	public function update_static_content_string( $replace_string, $content ) {
		if ( $this->has_static_content( $content ) ) {
			...
			$content = preg_replace( $this->regex_static_content, $replace_string, $content, 1 );
			$content = preg_replace( $this->regex_static_content, '', $content );
